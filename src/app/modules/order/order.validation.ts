...
const orderUpdateValidationSchema = z.object({
  user: z
  .string()
  .refine((val) => Types.ObjectId.isValid(val), 'Invalid user ObjectId').optional(),
  productDetails: z
    .string()
    .refine((val) => Types.ObjectId.isValid(val), 'Invalid car ObjectId').optional(), // Validates as a MongoDB ObjectId
